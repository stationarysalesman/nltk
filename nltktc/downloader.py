...
class Downloader(object):
    ...
    def _update_index(self, url=None):
        ...
        self._index = nltktc.internals.ElementWrapper(
    ...
    def default_download_dir(self):
        ...
        for nltkdir in nltktc.data.path:
            if (os.path.exists(nltkdir) and
                nltktc.internals.is_writable(nltkdir)):
                return nltkdir
...
def _find_packages(root):
    ...
    """
    from nltktc.corpus.reader.util import _path_from
    # Find all packages.
    packages = []
    for dirname, subdirs, files in os.walk(root):
