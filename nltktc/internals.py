...
from nltktc import __file__
from nltktc import compat
...
def config_java(bin=None, options=None, verbose=True):
    """
    Configure nltktc's java interface, by letting nltktc know where it can
    ...
    :param bin: The full path to the Java binary.  If not specified,
        then nltktc will search the system for a Java binary; and if
...
def read_str(s, start_position):
    ...
    :Example:
    >>> from nltktc.internals import read_str
...
def read_int(s, start_position):
    ...
    :Example:
    >>> from nltktc.internals import read_int
...
def read_number(s, start_position):
    ...
    :Example:
    >>> from nltktc.internals import read_number
...
def deprecated(message):
    ...
    a warning to be printed the when the function is used.  Usage:
        ...
        >>> from nltktc.internals import deprecated
...
def find_file_iter(filename, env_vars=(), searchpath=(),
    ...
    """
    Search for a file to be used by nltktc.
...
def find_jar_iter(name_pattern, path_to_jar=None, env_vars=(),
    ...
    """
    Search for a jar that is used by nltktc.
...
def import_from_stdlib(module):
    """
    When python is run from within the nltktc/ directory tree, the
    current directory is included at the beginning of the search path.
    Unfortunately, that means that modules within nltktc can sometimes
