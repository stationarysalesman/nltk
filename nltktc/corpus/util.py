...
class LazyCorpusLoader(object):
    ...
    def __load(self):
        ...
        if TRY_ZIPFILE_FIRST:
            try:
                root = nltktc.data.find('corpora/%s' % zip_name)
            except LookupError as e:
                try: root = nltktc.data.find('corpora/%s' % self.__name)
                except LookupError: raise e
        else:
            try:
                root = nltktc.data.find('corpora/%s' % self.__name)
            except LookupError as e:
                try: root = nltktc.data.find('corpora/%s' % zip_name)
                except LookupError: raise e
