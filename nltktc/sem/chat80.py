...
def _str2records(filename, rel):
    ...
    """
    recs = []
    contents = nltktc.data.load("corpora/chat80/%s" % filename, format="text")
...
def make_valuation(concepts, read=False, lexicon=False):
    ...
    if read:
        from nltktc.sem import Valuation
        val = Valuation({})
        val.update(vals)
