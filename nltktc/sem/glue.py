...
import nltktc
from nltktc.internals import Counter
from nltktc.compat import string_types
from nltktc.tag import UnigramTagger, BigramTagger, TrigramTagger, RegexpTagger
from nltktc.sem.logic import (Expression, Variable, VariableExpression,
                              LambdaExpression, AbstractVariableExpression)
from nltktc.compat import python_2_unicode_compatible
from nltktc.sem import drt
from nltktc.sem import linearlogic
...
class GlueDict(dict):
    ...
    def read_file(self, empty_first=True):
        ...
        try:
            contents = nltktc.data.load(self.filename, format='text', encoding=self.file_encoding)
            # TODO: the above can't handle zip files, but this should anyway be fixed in nltk.data.load()
        except LookupError as e:
            try:
                contents = nltktc.data.load('file:' + self.filename, format='text', encoding=self.file_encoding)
            except LookupError:
                raise e
...
class Glue(object):
    ...
    def train_depparser(self, depgraphs=None):
        if depgraphs:
            ...
            self.depparser.train_from_file(nltktc.data.find(
    ...
    def dep_parse(self, sentence):
        ...
        if self.depparser is None:
            from nltktc.parse import MaltParser
