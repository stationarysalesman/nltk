...
import re
from nltktc.compat import htmlentitydefs
...
def _join(lst, sep=' ', untag=False):
    ...
    try:
        ...
        from nltktc.tag import tuple2str
...
def in_demo(trace=0, sql=True):
    ...
    query.
    """
    from nltktc.corpus import ieer
    if sql:
        try:
            import sqlite3
...
def conllned(trace=1):
    ...
    from nltktc.corpus import conll2002
...
def ne_chunked():
    ...
    rels = []
    for i, sent in enumerate(nltktc.corpus.treebank.tagged_sents()[:1500]):
        sent = nltktc.ne_chunk(sent)
...
if __name__ == '__main__':
    import nltktc
    from nltktc.sem import relextract
    in_demo(trace=0)
    roles_demo(trace=0)
    conllned()
    conllesp()
