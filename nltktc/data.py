...
from nltktc.compat import py3_data, add_py3_data
from nltktc.compat import text_type, string_types, BytesIO, urlopen, url2pathname
...
def load(resource_url, format='auto', cache=True, verbose=False,
    ...
    if format == 'raw':
        ...
        import json
        from nltktc.jsontags import json_tags
        ...
        if format == 'text':
            ...
            resource_val = nltktc.grammar.CFG.fromstring(
                string_data, encoding=encoding)
        elif format == 'pcfg':
            resource_val = nltktc.grammar.PCFG.fromstring(
                string_data, encoding=encoding)
        elif format == 'fcfg':
            resource_val = nltktc.grammar.FeatureGrammar.fromstring(
                string_data, logic_parser=logic_parser,
                fstruct_reader=fstruct_reader, encoding=encoding)
        elif format == 'fol':
            resource_val = nltktc.sem.read_logic(
                string_data, logic_parser=nltktc.sem.logic.LogicParser(),
                encoding=encoding)
        elif format == 'logic':
            resource_val = nltktc.sem.read_logic(
                string_data, logic_parser=logic_parser, encoding=encoding)
        elif format == 'val':
            resource_val = nltktc.sem.read_valuation(
                string_data, encoding=encoding)
        else:
