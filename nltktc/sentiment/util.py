...
import nltktc
from nltktc.corpus import CategorizedPlaintextCorpusReader
from nltktc.data import load
from nltktc.tokenize.casual import EMOTICON_RE
from nltktc.twitter.common import outf_writer_compat, extract_fields
...
def extract_bigram_feats(document, bigrams):
    ...
    features = {}
    for bigr in bigrams:
        features['contains({0} - {1})'.format(bigr[0], bigr[1])] = bigr in nltktc.bigrams(document)
    return features
...
def demo_tweets(trainer, n_instances=None, output=None):
    ...
    """
    from nltktc.tokenize import TweetTokenizer
    from nltktc.sentiment import SentimentAnalyzer
    from nltktc.corpus import twitter_samples, stopwords
...
def demo_movie_reviews(trainer, n_instances=None, output=None):
    ...
    :param output: the output file where results have to be reported.
    """
    from nltktc.corpus import movie_reviews
    from nltktc.sentiment import SentimentAnalyzer
...
def demo_subjectivity(trainer, save_analyzer=False, n_instances=None, output=None):
    ...
    """
    from nltktc.sentiment import SentimentAnalyzer
    from nltktc.corpus import subjectivity
...
def demo_sent_subjectivity(text):
    ...
    """
    from nltktc.classify import NaiveBayesClassifier
    from nltktc.tokenize import regexp
...
def demo_liu_hu_lexicon(sentence, plot=False):
    ...
    """
    from nltktc.corpus import opinion_lexicon
    from nltktc.tokenize import treebank
...
def demo_vader_instance(text):
    ...
    """
    from nltktc.sentiment import SentimentIntensityAnalyzer
...
def demo_vader_tweets(n_instances=None, output=None):
    ...
    from nltktc.corpus import twitter_samples
    from nltktc.sentiment import SentimentIntensityAnalyzer
    from nltktc.metrics import (accuracy as eval_accuracy, precision as eval_precision,
...
if __name__ == '__main__':
    from nltktc.classify import NaiveBayesClassifier, MaxentClassifier
    from nltktc.classify.scikitlearn import SklearnClassifier
