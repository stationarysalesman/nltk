...
import nltktc
from nltktc.classify.util import accuracy
...
def lemmatize(word):
    ...
    """
    lemma = nltktc.corpus.wordnet.morphy(word, pos=nltktc.corpus.wordnet.VERB)
...
class RTEFeatureExtractor(object):
    ...
    def __init__(self, rtepair, stop=True, lemmatize=False):
        ...
        from nltktc.tokenize import RegexpTokenizer
...
def rte_classifier(trainer, features=rte_features):
    ...
    train = ((pair, pair.value) for pair in
             nltktc.corpus.rte.pairs(['rte1_dev.xml', 'rte2_dev.xml',
                                    'rte3_dev.xml']))
    test = ((pair, pair.value) for pair in
            nltktc.corpus.rte.pairs(['rte1_test.xml', 'rte2_test.xml',
                                   'rte3_test.xml']))
...
def demo_features():
    pairs = nltktc.corpus.rte.pairs(['rte1_dev.xml'])[:6]
    for pair in pairs:
        print()
...
def demo_feature_extractor():
    rtepair = nltktc.corpus.rte.pairs(['rte3_dev.xml'])[33]
    extractor = RTEFeatureExtractor(rtepair)
...
def demo():
    import nltktc
    try:
        nltktc.config_megam('/usr/local/bin/megam')
        trainer = lambda x: nltktc.MaxentClassifier.train(x, 'megam')
    except ValueError:
        try:
            trainer = lambda x: nltktc.MaxentClassifier.train(x, 'BFGS')
        except ValueError:
            trainer = nltktc.MaxentClassifier.train
    nltktc.classify.rte_classifier(trainer)
