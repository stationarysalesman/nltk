...
import nltktc.classify.util # for accuracy & log_likelihood
...
class CutoffChecker(object):
    ...
    def check(self, classifier, train_toks):
        ...
        new_ll = nltktc.classify.util.log_likelihood(classifier, train_toks)
        ...
        if 'max_acc' in cutoffs or 'min_accdelta' in cutoffs:
            new_acc = nltktc.classify.util.log_likelihood(
...
def names_demo(trainer, features=names_demo_features):
    from nltktc.corpus import names
    import random
...
def wsd_demo(trainer, word, features, n=1000):
    from nltktc.corpus import senseval
    import random
