...
>>> from nltktc.tree import ParentedTree
>>> from nltktc.tgrep import tgrep_nodes, tgrep_positions
...
from nltktc.compat import binary_type, text_type
import functools
import nltktc.tree
try:
    import pyparsing
except ImportError:
    print('Warning: nltktc.tgrep will not work without the `pyparsing` package')
    print('installed.')
import re
...
def _istree(obj):
    '''Predicate to check whether `obj` is a nltktc.tree.Tree.'''
    return isinstance(obj, nltktc.tree.Tree)
